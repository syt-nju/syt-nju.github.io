...
> Source: https://huggingface.co/docs/transformers/en/quantization/concept_guide
> Author: Hugging Face
> Collected: 2026-08-10
> Published: Unknown
> License: Unknown
> Completeness: Complete
> Retrieval note: Fetched from Transformers documentation HTML; navigation chrome removed.
...
The most common method is affine quantization. For a given float32 tensor (like a layer's weights), it finds the minimum val_min and maximum val_max values. This range [val_min, val_max] is mapped to the int8 range [q_min, q_max], which is typically [-128, 127].
...
Asymmetric (Affine): This method does not assume the data is centered around zero. It maps the exact range [val_min, val_max] from float32 to the full int8 range, like [-128, 127]. This requires two parameters, a scale (S) and a zero-point (Z).
...
However, int4 quantization typically results in a larger accuracy drop compared to int8. Advanced quantization techniques like GPTQ or AWQ are often necessary for good performance with int4.
...
Transformers supports FP8 through specific backends like FBGEMM, FineGrainedFP8, and compressed-tensors. These backends handle the underlying FP8 conversion and computation when the appropriate hardware and configurations are used.
...
Transformers integrates several quantization backends such as bitsandbytes, torchao, compressed-tensors, and more (refer to the quantization overview for more backends).

All backends are unified under the HfQuantizer API and associated QuantizationConfig classes. You can integrate your own custom quantization backends by implementing a custom HfQuantizer and QuantizationConfig, as shown in the Contribution guide.

The typical workflow for quantization in Transformers is to:

1. Choose a quantization method suitable for your hardware and use case (see the Overview or Selecting a quantization method guide to help you).
...
- Accelerating Generative AI with PyTorch Part 2: LLM Optimizations
