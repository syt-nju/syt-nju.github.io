...
大模型量化不是单一算法，而是一组互相耦合的选择：把哪些张量降到多少比特、用什么映射与粒度、误差在校准阶段还是训练阶段消化、以及最终服务场景是 GPU 推理、端侧部署，还是低显存微调。

本 topic 先回答四类问题：数值如何映射、PTQ 与 QAT 各自的流程边界、LLM 上主流 INT4 权重量化如何工作、以及后训练里 QLoRA 如何把 4-bit 基座与可训练 adapter 拼在一起。
