...
推理侧 **MXFP4** 也是 block-32：元素为 E2M1（1 sign / 2 exp / 1 mantissa），每 32 元共享 scale 以恢复动态范围；matmul 必须感知该 scale，依赖专用 kernel。详见 [MXFP4 推理量化](/wiki/llm-quantization/mxfp4/)。
...
MXFP4 则是 4-bit **浮点**权重路径：用 E2M1 + block scale，而不是 INT4 整数网格；与校准式 GPTQ/AWQ 不是同一条流水线。
