...
summary: "技能库是冻结 LLM 的外部程序性记忆；形态从 Voyager 代码、SKILL.md / SkillRepo、带单测的 Agent Skills，到 MCE 的可演化 CE skill 文件夹。"
lang: zh-CN
updated: 2026-08-05
order: 2
sources:
...
Skill library 把「做过一次的事」变成「以后还能调」的外部记忆。它不是原始对话回放，而是可检索、 ideally 可组合的程序性工件。表示形态随系统而变，但角色一致：在权重冻结时承载跨任务能力增长；在 [MCE](/wiki/skill-management/meta-context-engineering/) 中，同一文件夹抽象还被抬到「如何做 Context Engineering」一层。
...
上述表示都依赖检索或加载质量；库增长后噪声、冗余与过时条目会拖垮有效上下文。表示问题与治理问题应分开看：见生命周期、策展 RL，以及 MCE 用验证集做技能级选择的对照。
